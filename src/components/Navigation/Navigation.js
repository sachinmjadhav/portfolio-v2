...
function Navigation({ logoImg }) {
	...
	const whiteBackground = hasScrolledDown ? "white-background navbar-border" : "";
	const fontColor = hasScrolledDown ? "blue-font" : "white-font";
	
	return(
		<Navbar className={whiteBackground} fixed={"top"} expand="md">
			<div className="container">
				<NavbarBrand href="https://www.sachinjadhav.ooo" className={fontColor} style={{ display: 'flex', alignItems: 'center' }}>
					<Img
						fluid={logoImg.fluid}
						title="Logo"
						alt="Sachin Jadhav logo"
						className={classNames("img-responsive scrolledLogo", { logo: !hasScrolledDown })}
					/>
					SACHIN JADHAV
				</NavbarBrand>
